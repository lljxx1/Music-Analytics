...
const RESULT_SEPARATOR = 'xxxxxxxx'; // just a random separator to recognize when there are no more rows
...
class CliDatabase {
    ...
    execute(sql, callback) {
        ...
        try {
            console.log("execQueue", this.execQueue);
...
const database = new CliDatabase(
  "sqlite3",
  "/Users/fun/Library/Application Support/com.xiami.macclient/DataBase/Music.sqlite",
  (err) => {
 ...
 database.execute('select * from song_info', (_rows, err) => {
    console.log("_rows", _rows, err);
 });